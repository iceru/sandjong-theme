		...
		<?php if ( have_posts() ) : ?>
			<?php while ( have_posts() ) : the_post(); ?>
				...
				<header class="mb-10 md:mb-12">
					<h2 class="mb-4 max-w-4xl"><?php the_title(); ?></h2>
				</header>
				...
				<div class="grid md:grid-cols-[minmax(0,3fr)_minmax(0,1fr)] gap-10 md:gap-14 mb-16">
					<div>
						...
						<div class="flex justify-between items-center mt-4 text-sm body text-primary/80">
							<div class="flex flex-wrap gap-2">
								<?php
								$categories = get_the_category();
								if ( ! empty( $categories ) ) :
									foreach ( $categories as $cat ) : ?>
										<span class="uppercase tracking-wide text-terracota"><?php echo esc_html( $cat->name ); ?></span>
					...
					<aside class="pt-4 md:pt-2 border-t md:border-t-0 md:border-l border-[#CBAF8A]/60 md:pl-8">
						<h6 class="text-terracota mb-4">Share</h6>
							...
							<?php while ( $more_query->have_posts() ) : $more_query->the_post(); ?>
								<article>
									<div class="mb-6">
										...
										<?php else : ?>
											<a href="<?php the_permalink(); ?>" class="block overflow-hidden rounded-lg bg-primary/5 h-[220px] flex items-center justify-center">
									...
									<h5 class="mb-3">
										<a href="<?php the_permalink(); ?>" class="hover:text-terracota transition-colors"><?php the_title(); ?></a>
									</h5>
									<div class="body text-primary/80 mb-4"><?php echo wp_trim_words( get_the_excerpt(), 22 ); ?></div>
									<a href="<?php the_permalink(); ?>" class="inline-flex items-center body text-terracota hover:text-primary transition-colors">Read more</a>
								</article>
							<?php endwhile; ?>
						</div>
					</section>
